...
class ApplicationModel {
    ...
    static async findByEmail(email) {
        try {
            // 為了相容舊介面：先透過 users 表找出 user_id，再以 user_id 查 applications
            const [urows] = await db.query('SELECT user_id FROM users WHERE email = ? LIMIT 1', [email]);
            if (!urows || !urows[0]) return [];
    ...
    static async findByUserId(user_id) {
        try {
            const [rows] = await db.query('SELECT * FROM applications WHERE user_id = ?', [user_id]);
            return rows;
        } catch (error) {
            throw error;
        }
